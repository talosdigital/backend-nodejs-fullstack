...
exports.upload = function(req, res, next){
    ...
    fs.readFile(req.files.file.path, function(err, data){
        ...
        fs.writeFile("app/images/" + req.files.file.originalFilename, data, function(err) {
            if(err){
                res.send(500,err);
            }
            User.findById(userId, function (err, user) {
                user.avatar = "images/" + req.files.file.originalFilename;
                user.save(function(err) {
                    if (err) {
                        res.send(500, err);
                    } else {
                        res.send(200,{status:true,img:"images/" + req.files.file.originalFilename});
                    }
                });
            });
